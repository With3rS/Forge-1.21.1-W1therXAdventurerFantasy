...
public class ContaminatedEffect extends MobEffect implements ICustomStatusEffect {
    public ContaminatedEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public void onInitialisation(LivingEntity entity){
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.CONTAMINATED_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.CONTAMINATED_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.CONTAMINATED_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
                ...
                if (entry.data().contains("ticks")) {
                    ...
                    if (ticks >= 5) {
                        if (entity.level() instanceof ServerLevel level) {
                            level.sendParticles(ModParticles.CONTAMINATED_PARTICLES.get(), entity.getX(), entity.getY() + 0.5, entity.getZ(), 1, Math.random() * 0.5, Math.random(), Math.random() * 0.5, Math.random() * 0.5 + 0.1);
                            data.remove("ticks");
                            data.putInt("ticks", 0);
                            statsI.getActiveEffectList().replace(ModEffects.CONTAMINATED_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
                        }
