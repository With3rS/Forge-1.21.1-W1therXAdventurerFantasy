...
public class EcstaticEffect extends MobEffect implements ICustomStatusEffect {
    public EcstaticEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public void onInitialisation(LivingEntity entity){
        ...
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.ECSTATIC_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.ECSTATIC_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                if (!data.contains("angle")) {
                    data.putFloat("angle", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.ECSTATIC_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
    ...
    public void onTick(LivingEntity entity) {
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI-> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.ECSTATIC_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.ECSTATIC_EFFECT.get());
                CompoundTag data = entry.data();
                if (data.contains("ticks") && data.contains("angle")) {
                    int ticks = data.getInt("ticks");
                    if (ticks >= 3) {
                        if (entity.level() instanceof ServerLevel level) {
                            float angle = data.getFloat("angle");
                            level.sendParticles(ModParticles.ECSTATIC_PARTICLES.get(), entity.getX() + Math.cos(angle), entity.getY(), entity.getZ() + Math.sin(angle), 1, 0, Math.random(), 0, 0.1);
                            angle = (float) (angle + Math.PI/8);
                            if (angle >= 2*Math.PI) {
                                angle = 0F;
                            }
                            data.remove("ticks");
