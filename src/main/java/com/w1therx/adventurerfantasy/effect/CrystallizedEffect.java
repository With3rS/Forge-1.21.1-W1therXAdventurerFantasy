...
public class CrystallizedEffect extends MobEffect implements ICustomStatusEffect {
    public CrystallizedEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public void onInitialisation(LivingEntity entity){
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.CRYSTALLIZED_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.CRYSTALLIZED_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.CRYSTALLIZED_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
                ...
                if (data.contains("ticks")) {
                    ...
                    if (ticks >= 7) {
                        if (entity.level() instanceof ServerLevel level) {
                            data.remove("ticks");
                            data.putInt("ticks", 0);
                            level.sendParticles(ModParticles.CRYSTALLIZED_PARTICLES.get(), entity.getX(), entity.getY() + 0.5, entity.getZ(), 1, Math.random() * 0.4 + 0.1, Math.random(), Math.random() * 0.4 + 0.1, 0);
                            statsI.getActiveEffectList().replace(ModEffects.CRYSTALLIZED_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
                        }
                    } else {
                        data.remove("ticks");
