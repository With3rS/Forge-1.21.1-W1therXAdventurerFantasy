...
public class ResonantEffect extends MobEffect  implements ICustomStatusEffect {
    public ResonantEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public String effectDescription(LivingEntity entity) {
        return "Triggers Echo-related elemental reactions when paired with certain other elemental effects.";
    }

    @Override
    public void onInitialisation(LivingEntity entity) {
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.RESONANT_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.RESONANT_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.RESONANT_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
                ...
                if (entry.data().contains("ticks")) {
                    ...
                    if (ticks >= 10) {
                        if (entity.level() instanceof ServerLevel level) {
                            level.sendParticles(ParticleTypes.SCULK_CHARGE_POP, entity.getX(), entity.getY() + 0.5, entity.getZ(), 4, Math.random() * 0.8, Math.random() * 1.2, Math.random() * 0.8, Math.random() * 0.1 + 0.05);
                            data.remove("ticks");
                            data.putInt("ticks", 0);
                            statsI.getActiveEffectList().replace(ModEffects.RESONANT_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
                        }
