...
public class BlazingEffect extends MobEffect implements ICustomStatusEffect {
    public BlazingEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public String effectDescription(LivingEntity entity) {
        return "Triggers Fire-related elemental reactions when paired with certain other elemental effects.";
    }

    @Override
    public void onInitialisation(LivingEntity entity){
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.BLAZING_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.BLAZING_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.BLAZING_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
                ...
                if (data.contains("ticks")) {
                    ...
                    if (ticks >= 5) {
                        if (entity.level() instanceof ServerLevel level) {
                            data.remove("ticks");
                            data.putInt("ticks", 0);
                            level.sendParticles(ParticleTypes.LAVA, entity.getX(), entity.getY() + 0.5, entity.getZ(), 1, Math.random() * 0.5, Math.random(), Math.random() * 0.5, Math.random() * 0.5 + 0.1);
                            statsI.getActiveEffectList().replace(ModEffects.BLAZING_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
                        }
                    } else {
                        data.remove("ticks");
