...
public class NecroticEffect extends MobEffect  implements ICustomStatusEffect {
    public NecroticEffect(MobEffectCategory pCategory, int pColor) {
    ...
    public String effectDescription(LivingEntity entity) {
        return "Triggers Decay-related elemental reactions when paired with certain other elemental effects.";
    }

    @Override
    public void onInitialisation(LivingEntity entity) {
        entity.getCapability(ModCapabilities.INDEPENDENT_STATS).ifPresent(statsI -> {
            if (statsI.getActiveEffectList().containsKey(ModEffects.NECROTIC_EFFECT.get())) {
                StatusEffectInstanceEntry entry = statsI.getActiveEffectData(ModEffects.NECROTIC_EFFECT.get());
                CompoundTag data = entry.data();
                if (!data.contains("ticks")) {
                    data.putInt("ticks", 0);
                }
                statsI.getActiveEffectList().replace(ModEffects.NECROTIC_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
            }
        });
    }
                ...
                if (entry.data().contains("ticks")) {
                    ...
                    if (ticks >= 4) {
                        if (entity.level() instanceof ServerLevel level) {
                            level.sendParticles(ParticleTypes.ASH, entity.getX(), entity.getY() + 1.25, entity.getZ(), 4, Math.random() * 0.6, Math.random(), Math.random() * 0.6, Math.random() * 0.5 + 0.1);
                            data.remove("ticks");
                            data.putInt("ticks", 0);
                            statsI.getActiveEffectList().replace(ModEffects.NECROTIC_EFFECT.get(), new StatusEffectInstanceEntry(entry.duration(), entry.amplifier(), entry.stacks(), entry.maxStacks(), entry.applier(), data, entry.isInitialised()));
                        }
